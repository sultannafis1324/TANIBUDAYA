...
const AlamatEdit: React.FC = () => {
  ...
  const fetchAlamatData = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/alamat/${id}`, {
      ...
      if (data.provinsiId) {
        await fetchRegencies(data.provinsiId);
        if (data.kabupatenId) {
          await fetchDistricts(data.kabupatenId);
          if (data.kecamatanId) {
            await fetchVillages(data.kecamatanId);
          }
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/alamat/${id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
