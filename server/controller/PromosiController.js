import Promosi from '../models/Promosi.js'; // Sesuaikan path jika perlu
 ...
 * @access  Private (Admin)
 */
export const getAllPromosiForAdmin = async (req, res) => {
  try {
 ...
 * @access  Public
 */
export const getPublicPromosi = async (req, res) => {
  try {
    const now = new Date();
...
export const validatePromo = async (req, res) => {
  try {
    // Frontend harus mengirimkan data keranjang
    const { kode_promo, total_harga_produk, items } = req.body;
    
    if (!kode_promo || total_harga_produk === undefined || !items) {
    ...
    // 6. Hitung Diskon
    let nilai_diskon_dihitung = 0;
    
    if (promo.tipe_diskon === 'persen') {
      nilai_diskon_dihitung = totalEligible * (promo.nilai_diskon / 100);
      // Cek diskon maksimum
      if (promo.max_diskon > 0 && nilai_diskon_dihitung > promo.max_diskon) {
        nilai_diskon_dihitung = promo.max_diskon;
      }
    } else if (promo.tipe_diskon === 'nominal') {
      nilai_diskon_dihitung = promo.nilai_diskon;
      // Pastikan diskon tidak melebihi total
      if (nilai_diskon_dihitung > totalEligible) {
        nilai_diskon_dihitung = totalEligible;
      }
    } else if (promo.tipe_diskon === 'beli_x_gratis_y') {
      // Tipe 'Beli X Gratis Y' (cth: Beli 2 Gratis 1)
      // Ini HANYA bisa divalidasi jika frontend mengirim data item yang sesuai.
      // Untuk endpoint ini, kita anggap 'nilai_diskon' adalah 0
      // karena diskonnya berupa 'barang gratis' yg harus dihandle di cart.
      return res.status(400).json({ isValid: false, message: 'Tipe promo Beli-X-Gratis-Y tidak didukung via kode voucher' });
    }

    // 7. Kirim hasil validasi (Read-Only)
    // PENTING: Controller ini TIDAK menambah 'kuota_terpakai'.
    // Itu harus dilakukan di 'pesananController' dalam satu transaksi.
    res.status(200).json({
      isValid: true,
      message: 'Promo berhasil diterapkan!',
      id_promosi: promo._id,
      kode_promo: promo.kode_promo,
      nilai_diskon: Math.round(nilai_diskon_dihitung) // Bulatkan
    });
